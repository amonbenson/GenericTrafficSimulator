...
public abstract class ConsolePane extends JPanel implements MouseListener {
	...
	public void updatePreferredSize() {
		...
		Dimension size = new Dimension(GraphicsFX.highDPI(DEFAULT_WIDTH), lines.size() * dy + sy * 2);

		// If the pane is already that size, return (don't wanna paint things
		// more than once and cause lag)
			...
			for (String line : lines) {
				...
				for (String part : parts) {
					if (part.startsWith("%%")) part.replace("%%", "%");
						...
						// Set the color
						g.setColor(new Color((hex & 0xFF0000) >> 16, (hex & 0xFF00) >> 8, (hex & 0xFF)));
