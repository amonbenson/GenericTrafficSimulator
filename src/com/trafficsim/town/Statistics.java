...
public class Statistics {
	...
	//Fehler, wenn keine Route erzeugt wird, weil der Weg zu umst�ndlich ist.
	 ...
	 * In TravelTime sind nur die erfolgreichen Transporte geloggt, daher m�ssen noch die Fehler+
	 * nicht transportierten Personen hinzugef�gt werden.
	 * @return
	 */
	public float getAverageTravelTime(Town t) {
		BigDecimal sumBig = new BigDecimal(0);
		long sum = 0;
		for (RouteTime r : travelTimes) { //Erfolgreiche Transporte hinzuf�gen
		...
		for ( Bus b : busses ) {
			...
			for ( Person p : persons ) {
				...
				sum += (t.getTime() - p.getTimeStart()); //F�gt die Differenz hinzu, wie lange der Mensch schon wartet
				personCounter++;
			}
		}
			//in Stationen:
	...
	public float getMedianTravelTime(Town t) {
		...
		//TravelTimes (erfolgreiche Transporte) hinzuf�gen
		...
		for ( Bus b : busses ) {
			ArrayList<Person> persons = b.getPersons();
			for ( Person p : persons ) {
				times.add(t.getTime() - p.getTimeStart()); //F�gt die Differenz hinzu, wie lange der Mensch schon wartet
			}
		}
		//in Stationen:
		ArrayList<StreetTile> streets = t.getStreetTiles();
		...
		if (times.size() % 2 == 0) { //gerade Anzahl, arithmetisches Mittel vom Unter und Obermedian wird ben�tigt
			long lower, upper;
			if (times.size() == 2) {
				lower = times.get(0);
				upper = times.get(1);
